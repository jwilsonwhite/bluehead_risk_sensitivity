...
#-----------------------------------------------------------------------------------
create.scatterplot<-function(D,m,ylab=NULL,xlab=NULL,Month=FALSE){
  # Helper function to create a scatterplot. Only works in context of the lmer objects used in this code
if (Month){
  Months = c('7','8')
  # intervals:
  D$fitted <- NA
  ...
  D$lm.lower[OKm] <- D$fitted[OKm] - 1.96*SEline.mean
  D$lm.upper[OKm] <- D$fitted[OKm] + 1.96*SEline.mean
  }
...
D$lm.lower <- D$fitted - 1.96*SEline.mean
D$lm.upper <- D$fitted + 1.96*SEline.mean
# plot:
